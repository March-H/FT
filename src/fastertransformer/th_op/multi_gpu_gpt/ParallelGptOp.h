...
#include "src/fastertransformer/scheduling/Job.h"
...
public:
    ...
    virtual void forward(th::Tensor&              input_ids,
                         ...
                         th::optional<th::Tensor> additional_msg = th::nullopt) = 0;
};
...
public:
    ...
    void forward(th::Tensor&              input_ids,
                 ...
                 th::optional<th::Tensor> bad_words_list_opt,
                 th::optional<int64_t>    return_cum_log_probs_opt,
                 th::optional<th::Tensor> additional_msg = th::nullopt) override
    {
        ...
        if(additional_msg != th::nullopt && additional_msg.has_value()){
            input_tensors.insert({"additional_msg", convert_tensor<int>(additional_msg.value())});
        }

//        th::Tensor session_len = th::tensor(2048);
//
//        input_tensors.insert(
//                {"session_len", convert_tensor<int>(session_len, ft::MemoryType::MEMORY_CPU)});



        try {
            if(Config::FAST_SERVE_SCHEDULER){
//                FT_LOG_INFO("first step.");
                auto job = scheduler.allocate_new_job(additional_msg.value()[0].item<int>(), (int)max_input_length);
//                FT_LOG_INFO("second step.");
                gpt.forward(&output_tensors, &input_tensors, &gpt_weights_, job);
            }else{
                Job job(0,0);
                gpt.forward(&output_tensors, &input_tensors, &gpt_weights_, &job);
            }
...
public:
    ...
    vector<th::Tensor> forward(th::Tensor               input_ids,
                               ...
                               th::optional<th::Tensor> bad_words_list_opt,
                               th::optional<int64_t>    return_cum_log_probs_opt,
                               th::optional<th::Tensor> additional_msg = th::nullopt);
