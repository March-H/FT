...
//            FT_LOG_INFO("self_attention_layer part is running, cost %lfms",(double)(end-start).count()/(1000*1000));
...
//            FT_LOG_INFO("has_adapters_ is %d",has_adapters_);
            // 虽然不知道这一块是啥，但是下面这个if不会执行
            if (has_adapters_) {
                ...
                auto end = std::chrono::high_resolution_clock::now();
                FT_LOG_INFO("ffn_layer is working, cost %lf",(double)(end-start).count()/(1000*1000));
            }
...
//            FT_LOG_INFO("ffn_layer part is running, cost %lfms",(double)(end-start).count()/(1000*1000));
